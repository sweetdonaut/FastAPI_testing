...
## 第二階段：壓力測試開發（2024-06-17 續）

### 目標
創建更具挑戰性的測試圖片，評估檢測器在極端條件下的性能

### 主要挑戰
1. **邊緣模糊**：真實 SEM 影像的邊緣不會很銳利
2. **正確定位**：缺陷必須在垂直邊緣與水平亮條交界處
3. **橢圓形狀**：更符合真實缺陷形態
4. **邊界排除**：100像素邊界區域不考慮
...
#### 突破：梯度分析方法
- 使用 analyze_vertical_structure.py 詳細分析
- 區分 Dark→Bright 和 Bright→Dark 邊緣
- 找到正確的垂直邊緣位置

#### 最終解決方案
- create_final_blurred_defects.py：
  - 準確的垂直邊緣檢測
  - 硬編碼的水平亮條位置
  - 強模糊處理（sigma=2.5）
  - 橢圓形缺陷
...
### 第二階段檔案整理
所有中間版本已移至 old_versions/（編號 12-24）

## 下一步計畫
1. 使用 sem_defect_detector_v1.py 測試模糊邊緣圖片
2. 分析檢測失敗的原因
3. 考慮改進方案（預處理、動態閾值等）
