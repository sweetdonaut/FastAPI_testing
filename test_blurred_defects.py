...
def test_single_defect(detector, defect_info, save_prefix='test_defect'):
    """測試單個缺陷"""
...
def create_summary_visualization(test_results, image_path='final_blurred_defects.jpg'):
    ...
    plt.title('Blurred Defects Detection Summary', fontsize=16, weight='bold')
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('blurred_detection_summary.png', dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\nSummary visualization saved as 'blurred_detection_summary.png'")
...
if __name__ == "__main__":
    ...
    # 初始化檢測器
    print("\nInitializing detector with blurred image...")
    detector = SEMDefectDetector('final_blurred_defects.jpg')
    
    # 測試所有缺陷
    test_results = []
    for defect in defects:
        result = test_single_defect(detector, defect, save_prefix='blurred_test')
